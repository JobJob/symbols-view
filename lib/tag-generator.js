...
export default class TagGenerator {
  ...
  getPackageRoot() {
    ...
    const packageRoot = currentFileWasRequiredFromSnapshot
      ? path.join(resourcePath, 'node_modules', 'symbols-view-click')
      : path.resolve(__dirname, '..');

    if (path.extname(resourcePath) === '.asar' && packageRoot.indexOf(resourcePath) === 0) {
      return path.join(`${resourcePath}.unpacked`, 'node_modules', 'symbols-view-click');
    } else {
      return packageRoot;
    }
  }
  ...
  generate() {
    ...
    if (atom.config.get('symbols-view-click.useEditorGrammarAsCtagsLanguage')) {
